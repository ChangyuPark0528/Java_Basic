...
public class ArrayListExample {
	...
	public static void main(String[] args) {
        ...
        */
		...
		List<String> nick = new ArrayList<>(); //변수 타입을 인터페이스(import)로 지정한다.
		...
		//18행에서 제네릭 안에 String으로 지정했기 때문에 현재 전부 타입이 Srting으로 지정되어있다.
		...
		//indexOf: 특정 객체의 인덱스 탐색 / ()작성하지 않으면 true / false
		int idx = nick.indexOf("메롱"); //해당객체가 없으면 -1 return
...
//		List<int> score = new ArrayList<>(); 
		List<Integer> score = new ArrayList<>(); //객체를 사용하기때문에 제네릭의 해당값은 객체타입으로 작성해야한다.
		score.add(65);
		
		//한번에 객체 많이 추가하기
		Collections.addAll(score, 78, 100, 87, 88, 952, 64, 64, 87);
		System.out.println(score);
		
		//컬렉션에서의 등장 횟수 구하기
		System.out.println("100점 학생의 수: " + Collections.frequency(score, 100));
...
//		Collections.reverse(score); //역순 정렬 
		Collections.sort(score, Collections.reverseOrder()); //정렬 후 내림차
		System.out.println(score);
