...
import etc.api.lang.system.systemTime;

public class MainClass {
	
	// 내부 클래스(inner class)
	// 해당 객체와 밀접한 연관이 있는 경우 클래스 내에서 클래스 선언.
	//일반적인 class를 정의할때는 private을 사용할 수 없지만
	//클래스 내에 클래스를 선언하는 경우에는 private 선언이 가능
	//static -> 메모리 효율을 높이기 위해 선언.(바깥쪽 클래스 호출 시마다 재생성하는것을 방지)
	...
	public static void main(String[] args) {
		...
		};//일회용의 느낌으로 클래스 안에서만 사용할 수 있는 클래스
		
		System.out.println(multiCal.operate(5, 7));
		
		// 나눗셈 계산을 하는 계산기를 만들자 (람다) -> 인터페이스의 추상메서드가 딱 1개일 경우만 가능
		// 어차피 인터페이스에 추상메서드가 1개이기 때문에 이름을 따로 명시하지 않음
		// 메서드 내부 로직이 단 한줄이라면 괄호 생략가능 -> return 값으로 취급.
