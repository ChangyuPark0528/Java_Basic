...
//	void inputData(int b, int a) {} (x) 19행의 메서드와 매개변수 타입이 int 2개로 중복
	
	void inputData(String s) {}
	
	void inputData(int a, double d) {} 
	
	void inputData(double d, int a) {} //매개 변수의 전달 순서가 다르기때문에 가능.
	
//	void inputData(int number) {} (x) 정수 매개값 1개를 받는 메서드가 이미 선언됨.
	
//	int inputData(int number) {
//		return 0;
//	} (x) 반환유형 (return type)은 오버로딩에 영향을 미치지못함.
...
//	}
//	
//	double calcRectArea(int ceil, int width, int height) {
//		return ()
//	}
